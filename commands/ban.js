  ...
  if (auteur.hasPermission('BAN_MEMBERS')){
    ...
    if (user) {
      ...
      if (member) {
        ...
        await member.createDM().then(ch => ch.send(`Vous avez été frappé par le banhammer, asséné part ${msg.author.tag}, pour la raison : "${reason}".`).catch(hook.send("Je n'est pas pu envoyer de message à la personne banni :/")));
